# Survival Function with DeSolve
...
fname = c('Survival_Left_Mendota2019.Rdata')
...
dx = diff(range(xs))/N  # increment of x on the mesh
...
windows()
par(mfrow=c(1,1),mar=c(4,4.5,2,2)+0.1,cex.axis=1.6,cex.lab=1.6)
plot(zhalf$y,zhalf$x,type='l',lwd=2,xlab='X',ylab='S(x,t) = 0.5 (min)')
...
windows()
par(mfrow=c(1,1),mar=c(4,4.5,2,2)+0.1,cex.axis=1.6,cex.lab=1.6)
plot(zhalf$y,zhalf$x,type='l',lwd=2,col='blue',xlab='X',ylab='Half-Life, minutes',
