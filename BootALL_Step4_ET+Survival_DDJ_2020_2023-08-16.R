...
# Enter Data - Results of bootstrap DDJ ----------------------------------------------------
#save(year,Nboot,amat,D1mat,D2mat,sigmat,xeqmat,EPxeqmat,file=Fname)
load(file='DDJ_boot_2020.Rdata')
fname=c('ET+Sboot_2020.Rdata')
year = 2020

aropt = 5  # data were thinned before DDJ so time step is aropt*(DT of data) for the rates

# Simulation control parameters
tstart = Sys.time() # start clock

print(c('starting ET bootstrap for ',year),quote=F)

ETLR = matrix(0,nr=Nboot,nc=2)
ShalfLR = matrix(0,nr=Nboot,nc=2)

ETfun = for(ib in 1:Nboot)  {  # usually 1:NOK
  xeq = EPxeqmat[,ib]
  D1x = amat[,ib]
  D1y = D1mat[,ib]
  D2x = amat[,ib]
  D2y = D2mat[,ib]  # total variance from Johannes
  ...
  x = seq( -5.5,xeq[2],length.out=20) # mesh of 20 (vs 30) does not blow up bvptwp
  ...
  x = seq(xeq[2],5.5,length.out=20) # mesh of 20 (vs 30) does not blow up bvptwp
  # solve with bvpcol or bvptwp
  trycol <- tryCatch({
    bvptwp(yini = yini, x = x, func = feval2, yend = yend, parm=plist)
  },error=function(e){cat("ERROR :",conditionMessage(e), "\n")} )
  ...
  ETLR[ib,] = c(meanETl,meanETr) # output result
  tnow = Sys.time()
  etime = difftime(tnow,tstart,units='mins')
  print(c(ib,ETLR[ib,],etime),quote=F) 
  ...
  xs = seq(from=xrange[1],to=xrange[2],length.out=N) # left basin # left basin
  dx = diff(range(xs))/N
  ...
  ShalfLR[ib,1] = Shalf.left
  print(c('For step ',ib,' Weighted Half-Life, Left ',Shalf.left),quote=F)
  ...
  ShalfLR[ib,2] = Shalf.right
  print(c('For step ',ib,' Weighted Half-Life, Right ',Shalf.right),quote=F)
  print('',quote=F)
}
...
# use thinning interval to convert time steps to minutes
ETLR = aropt*ETLR
ShalfLR = aropt*ShalfLR

print('mean and s.d. for L and R bootstrapped exit times',quote=F)

etlr.mean = apply(ETLR,MARGIN=2,FUN='mean',na.rm=T)
etlr.sd = apply(ETLR,MARGIN=2,FUN='sd',na.rm=T)

print(etlr.mean)
print(etlr.sd)

print('mean and s.d. for L and R bootstrapped half live',quote=F)

shalf.lr.mean = apply(ShalfLR,MARGIN=2,FUN='mean',na.rm=T)
shalf.lr.sd = apply(ShalfLR,MARGIN=2,FUN='sd',na.rm=T)
